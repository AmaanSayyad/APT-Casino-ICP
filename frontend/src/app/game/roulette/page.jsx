...
import { gameData, bettingTableData } from "../mines/config/gameDetail";
...
export default function GameRoulette() {
  ...
  const winningsListener = useMemo(() => {
    return publicMantleSepoliaClient.watchContractEvent({
      ...
      onLogs: (logs) => {
        const randomNumber = parseInt(logs[0].args.randomNumber);
  ...
  useEffect(() => {
    const checkNetwork = async () => {
      ...
      if (window.ethereum) {
        const currentChaidId = await window.ethereum.request({
          method: "eth_chainId",
        });
        if (currentChaidId === "0x138b") {
      ...
      try {
        ...
        if (switchError.code === 4902) {
          try {
            ...
            // After adding the chain, switch to it
            await window.ethereum.request({
              method: "wallet_switchEthereumChain",
              params: [{ chainId: "0x138b" }],
            });
