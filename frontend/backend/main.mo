...
actor class Roulette() = this {
    ...
    public shared({ caller }) func placeBet(bets : [Bet]) : async Result.Result<(), Text> {
        ...
        let playerState = Option.get(Trie.get(currentRound, key(caller), Principal.equal), {
            bets = List.nil();
            hasPlaced = false;
        });
    ...
    func checkWin(bet : Bet, winningNumber : Nat8) : Bool {
        switch (bet.betType) {
            ...
            case (#evenOdd isEven) { 
                if (winningNumber == 0) false 
                else { winningNumber % 2 == (if isEven 0 else 1) }
            };
            case (#lowHigh isLow) {
                if (winningNumber == 0) false
                else { (winningNumber <= 18) == isLow }
            };
        }
    };
    ...
    func checkSplit(n : Nat8, wn : Nat8) : Bool {
        ...
        Array.find<[Nat8]>(splits, func(pair) {
            (pair[0] == n and pair[1] == wn) or 
            (pair[1] == n and pair[0] == wn)
        }) != null
    };
    ...
    func checkCorner(n : Nat8, wn : Nat8) : Bool {
        ...
        Array.find<[Nat8]>(corners, func(corner) {
            Array.find<Nat8>(corner, func(num) { num == wn }) != null and 
            Array.find<Nat8>(corner, func(num) { num == n }) != null
        }) != null
    };
